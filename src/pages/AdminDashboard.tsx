    ...
    <div className="py-24 px-4 bg-slate-950 min-h-[80vh] border-b border-blue-500/20">
      <div className="max-w-7xl mx-auto">
        <div className="mb-16 flex flex-col items-center text-center">
          <h1 className="text-4xl font-serif text-slate-100 mb-4">Admin Portal</h1>
          <p className="text-[11px] uppercase tracking-widest text-slate-400 mb-12">Manage appointments and salon schedules.</p>
            ...
            {['all', 'Pending', 'Approved', 'Cancelled'].map(f => (
              <button
                ...
                className={`px-8 py-3 text-[10px] font-bold uppercase tracking-[0.1em] transition-colors border ${
                  filter === f ? 'bg-blue-500 text-white border-blue-500' : 'bg-transparent text-slate-400 border-slate-800 hover:border-blue-500 hover:text-blue-500'
                }`}
              >
                {f}
              </button>
            ))}
            <div className="w-px h-8 bg-slate-900 mx-2 hidden md:block"></div>
            <button
              onClick={() => setIsModalOpen(true)}
              className="px-6 py-3 text-[10px] font-bold uppercase tracking-[0.1em] transition-colors border border-slate-700 bg-blue-600 text-white hover:bg-blue-700 flex items-center gap-2"
            >
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              ...
              <tbody className="divide-y divide-neutral-100">
                {filteredApps.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="p-16 text-center text-slate-400 text-[11px] uppercase tracking-[0.2em]">No appointments found.</td>
                  </tr>
                ) : (
                  filteredApps.map(app => (
                    <tr key={app.id} className="hover:bg-slate-900 transition-colors">
                      <td className="p-8">
                        <p className="font-serif text-lg text-slate-100">{app.name}</p>
                      </td>
                      <td className="p-8">
                        <p className="font-serif text-slate-300">{app.serviceName}</p>
                      </td>
                      <td className="p-8">
                        <div className="flex flex-col gap-2 text-[10px] uppercase tracking-[0.1em] text-slate-400">
                      ...
                      <td className="p-8">
                        <div className="flex flex-col gap-2 text-[10px] uppercase tracking-[0.1em] text-slate-400">
                          <p>{app.phone}</p>
                          <p className="text-slate-400 overflow-hidden text-ellipsis">{app.email}</p>
                        </div>
                      </td>
                      <td className="p-8">
                        ...
                        <div className="flex justify-end gap-4">
                          {app.status !== 'Approved' && (
                            <button
                              ...
                              className="text-green-400 hover:text-green-300 font-bold uppercase tracking-widest text-[9px] border border-green-500/50 px-3 py-2 hover:bg-green-500/20 transition-colors bg-slate-950"
                              title="Approve"
                            >
                              Approve
                            </button>
                          ...
                          {app.status !== 'Cancelled' && (
                            <button
                              ...
                              className="text-blue-400 hover:text-blue-300 font-bold uppercase tracking-widest text-[9px] border border-blue-500/50 px-3 py-2 hover:bg-blue-500/20 transition-colors bg-slate-950"
                              title="Cancel"
                            >
                              Cancel
                            </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-sm">
          <div className="bg-slate-950 border border-blue-500/20 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-slate-950 border-b border-blue-500/20 px-8 py-6 flex justify-between items-center z-10">
              <h2 className="text-2xl font-serif text-slate-100">New Appointment</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-slate-100 transition-colors px-2 py-2">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit(handleCreateAppointment)} className="p-8 space-y-8">
              <div className="grid md:grid-cols-2 gap-8">
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-2">Service</label>
                  <select 
                    {...register('serviceId')} 
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-2">Guest Name</label>
                  <input 
                    type="text" 
                    {...register('name')} 
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-2">Phone</label>
                  <input 
                    type="tel" 
                    {...register('phone')} 
                    className="w-full py-3 bg-transparent border-b border-slate-800 text-slate-100 text-sm outline-none focus:border-blue-500 transition-colors placeholder:text-slate-300"
                ...
                <div className="md:col-span-2">
                  <label className="block text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-2">Email (Optional)</label>
                  <input 
                    type="email" 
                    {...register('email')} 
                    className="w-full py-3 bg-transparent border-b border-slate-800 text-slate-100 text-sm outline-none focus:border-blue-500 transition-colors placeholder:text-slate-300"
